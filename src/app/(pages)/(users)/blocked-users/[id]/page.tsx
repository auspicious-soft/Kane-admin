...
import { getUserById, getUserOfferAndRedemptionHistory } from "@/services/admin-services";
  ...
  useEffect(() => {
     const fetchUserDetails = async () => {
       try {
         ...
         setLoading(true);
         startLoading();
         const response = await getUserById(`${USER_URLS.GET_SINGLE_USER(id as string)}`);
         ...
         const mappedUser: User = {
           ...
           reasonForBlock:userDetails.reasonForBlock ? userDetails.reasonForBlock : null,
           ...
           date: userDetails.createdAt ? new Date(userDetails.createdAt).toLocaleDateString() : "", 
         };
...
useEffect(() => {
  const fetchRedemptionHistory = async () => {
    try {
      setLoading(true);
      startLoading();
      const currentPage = type === "offer" ? offerCurrentPage : redemptionCurrentPage;
      ...
      if (type === "offer") {
        const mappedOfferHistory: OffHistory[] = historyData.map((item: any) => ({
          id: item._id,
        ...
        const mappedRedemptionHistory: RedemptHistory[] = historyData.map((item: any) => ({
          id: item._id,
  ...
  return (
    <>
      ...
      <Tabs 
      defaultValue="redemptionHistory" 
      className="w-full"
